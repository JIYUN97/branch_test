...
exports.createComment = async (req, res) => {
  ...
  try {
    ...
    //후기 작성 여부
    if (commentCheck)
      return res.status(401).send({ err: "이미 해당 상품의 후기를 작성하셨습니다." })
    //해당 상품 구매여부
    if (!buyCheck)
      return res.status(401).send({ err: "구매한 상품에 대해서만 후기 작성 가능합니다." })
...
exports.deleteComment = async (req, res) => {
  ...
  try {
    ...
    if (goods.comment_count == 0)
      return res.status(400).send({ err: "해당 상품의 후기가 존재하지 않습니다." })
    //댓글 삭제 및 댓글 갯수 수정
    await Promise.all([
